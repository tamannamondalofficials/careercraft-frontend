  ...
  return (
    <header className="h-16 bg-white border-b border-gray-200 px-6 md:px-8 lg:px-10 flex items-center justify-between shrink-0 shadow-xs z-30 box-border">
      ...
      <div className="flex items-center gap-2.5 sm:gap-3 shrink-0">
        ...
        {/* Backend Submit / Save Button */}
        <button
          onClick={onBackendSubmit}
          disabled={isSubmitting}
          className="hidden sm:flex items-center gap-1.5 text-xs font-semibold text-gray-700 bg-white hover:bg-gray-50 border border-gray-300 px-3.5 py-2 rounded-xl shadow-2xs hover:border-gray-400 transition-all cursor-pointer active:scale-95 disabled:opacity-50"
          title="Save resume to backend database (http://127.0.0.1:8000/api/resumes)"
        >
          {isSubmitting ? (
          ...
          <span>{isSubmitting ? 'Saving...' : 'Save to DB'}</span>
        </button>
        ...
        <div className="flex md:hidden bg-gray-100 p-1 rounded-lg border border-gray-200">
          <button
            ...
            className={`flex items-center gap-1 px-3 py-1 text-xs font-medium rounded-md transition-all cursor-pointer ${mobileTab === 'edit' ? 'bg-white text-gray-900 shadow-xs' : 'text-gray-600'
              }`}
          >
            <Edit3 size={13} />
            Edit
          ...
          <button
            ...
            className={`flex items-center gap-1 px-3 py-1 text-xs font-medium rounded-md transition-all cursor-pointer ${mobileTab === 'preview' ? 'bg-white text-gray-900 shadow-xs' : 'text-gray-600'
              }`}
          >
            <Eye size={13} />
            Preview
